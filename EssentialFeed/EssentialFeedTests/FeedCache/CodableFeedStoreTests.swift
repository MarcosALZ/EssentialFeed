...
//  EssentialFeed
//
//  Created by Israel Marcos Alvarez Mesa on 7/10/25.
//
...
class CodableFeedStoreTests: XCTestCase {
    ...
    func test_insert_overridesPreviouslyInsertedCacheValues() {
        ...
        let firstInsertionError = insert(cache: (uniqueImageFeed().local, timestamp: Date()), to: sut)
        XCTAssertNil(firstInsertionError, "Expected to insert cache successfully")
        
        let latestFeed = uniqueImageFeed().local
        let latestTimestamp = Date()
        
        let secondInsertionError = insert(cache: (latestFeed, latestTimestamp), to: sut)
    ...
    func test_insert_deliversErrorOnInsertionError() {
        ...
        let insertionError = insert(cache: (feed, timestamp: timestamp), to: sut)
    ...
    @discardableResult
    private func insert(cache: (feed: [LocalFeedImage], timestamp: Date), to sut: CodableFeedStore, file: StaticString = #filePath, line: UInt = #line) -> Error? {
    ...
    private func expect(_ sut: CodableFeedStore, toRetrieve expectedResult: RetrieveCacheFeedResult, file: StaticString = #filePath, line: UInt = #line) {
        ...
        sut.retrieve { retrievedResult in
            ...
            case (.empty, .empty),
                (.failure, .failure):
                break
            case let (.found(expectedResultFeed, timestamp: expectedResultTimestamp), .found(retrievedResultFeed, timestamp: retrievedResultTimestamp)):
                XCTAssertEqual(expectedResultFeed, retrievedResultFeed)
                XCTAssertEqual(expectedResultTimestamp, retrievedResultTimestamp)
    ...
    private func setupUpEmptyStoreState() {
